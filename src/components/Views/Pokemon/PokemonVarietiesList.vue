...
export default class PokemonVarietiesList extends Vue {
    ...
    async changeVariation(variety: number) {
    ...
    calculateStats(variety: PokemonData) {
        ...
        this.varieties[this.currentVarietyIndex].pokemonFull.stats.forEach(
            stat => {
                stats[stat.stat.name] -= stat.base;
                if (stats[stat.stat.name] === 0) delete stats[stat.stat.name];
            }
        );
        return stats;
    }
